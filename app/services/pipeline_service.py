...
def execute_pipeline(run_id: int) -> None:
    ...
    try:
        ...
        try:
            ...
            pipeline_repository.finish_step(
                db, step2.id,
                success=True,  # même s'il y a des issues, l'étape elle-même n'échoue pas
                output=summary,
            )
        ...
        # ─── Fin (succès) — l'IA et le build viendront aux étapes suivantes ───
        pipeline_repository.update_run_status(db, run_id, PipelineStatus.success)
        logger.info(f"[pipeline] Run #{run_id} terminé avec succès")
